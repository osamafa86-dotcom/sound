...
/** كثافة التشكيل: نسبة الحركات إلى الحروف العربية — مقياس «مشكّل عمداً» */
export function vocalizationDensity(text: string): number {
  const letters = text.match(ARABIC_LETTERS)?.length ?? 0;
  if (!letters) return 0;
  return (text.match(ARABIC_MARKS)?.length ?? 0) / letters;
}

/**
 * عتبة «المشكّل عمداً»: نص تجاوزها شكّله صاحبه أو المساعد (بنموذج أعلى)
 * أو ملقّنٌ سابق — إعادة تلقينه بنموذج أخف تفسد الجيد وتراكم الانجراف
 * بين النسخ، فيمرّ كما هو حرفياً. تحتها (عارٍ أو جزئي) يعمل الملقّن.
 */
export const DELIBERATE_TASHKEEL_DENSITY = 0.5;
...
/**
 * قاموس الكلمات الوظيفية القصيرة بنطقها الغنائي القياسي — حرف جر + ضمير:
 * نهايتها العارية تدعو المحرك للارتجال (سُمع «بهر» بدل «بِه» على الإنتاج)،
 * وحركة نهاية صريحة تسد الباب. الحروف نفسها لا تتغير أبداً.
 */
const FUNCTION_WORD_ENDINGS: [string, string][] = [
  ["به", "بِهِ"],
  ["بها", "بِهَا"],
  ["بهم", "بِهِمْ"],
  ["له", "لَهُ"],
...
/**
 * إتمام تلقيني حتمي للكلمات الوظيفية عارية النهاية — يعمل دائماً حتى على
 * النص المشكّل عمداً (فهو إكمالٌ موضعي لا إعادة تشكيل): «بِه» ⟵ «بِهِ».
 * كلمة نهايتها محرّكة أصلاً (بِهِ أو بِهْ وقفاً متعمداً) لا تُمس.
 */
export function completeFunctionWords(text: string): string {
  const map = new Map(FUNCTION_WORD_ENDINGS);
  return text.replace(/[ء-يً-ْٰـ]+/g, (token) => {
    // النهاية المحرّكة (أو المسكّنة وقفاً) اختيار متعمد — يُحترم
    if (/[ً-ْٰ]/.test(token[token.length - 1])) return token;
    const skeleton = token.replace(/[ً-ْٰـ]/g, "");
    const direct = map.get(skeleton);
    if (direct) return direct;
    // بادئة العطف الملتصقة (و/ف): «وعليه» ⟵ «وَعَلَيْهِ» — البادئة المحرّكة تبقى بحركتها
    const m = token.match(/^([وف][ً-ْٰ]*)(.+)$/);
    if (m) {
      const rest = map.get(m[2].replace(/[ً-ْٰـ]/g, ""));
      if (rest) return (/[ً-ْٰ]/.test(m[1]) ? m[1] : `${m[1]}َ`) + rest;
    }
    return token;
  });
}
...
function buildPrompt(sections: SongSection[], dialectId: string): string {
  ...
  return `أنت مُلقِّن أداء غنائي محترف (vocal coach) وخبير عروض متخصص في اللهجة ${dialect.name}. أمامك مقاطع أغنية ستُغنّى آلياً — المحرك ينطق النص كما كُتب حرفياً حرفاً حرفاً، فمهمتك تلقينه النطق الكامل الدقيق بالتشكيل حتى يخرج الأداء سليماً ١٠٠٪:
...
٢. **شكّل كل كلمة تشكيلاً تاماً كما تُنطق باللهجة ${dialect.name} لا بالفصحى** — بالحركات وحدها: الكسرة والفتحة والضمة كما ينطقها أهل اللهجة فعلاً، والسكون العامي في أواسط الكلمات ونهاياتها يُثبت صراحة.
٣. **تلقين الأداء حسب لغة النص — وهذا فرق جوهري**:
   - **نص فصيح (أو اللهجة فصحى)**: أعرِب إعراباً غنائياً صحيحاً كما في الطرب الكلاسيكي — التنوين يبقى حيث يستقيم عروضياً ويُغنّى، والقافية المطلقة تُمَدُّ بحركتها الطويلة ولا تُسكَّن قسراً، والوقف بالسكون على القافية المقيدة وحدها. لا تعامل الفصحى بقواعد العامية أبداً.
   - **نص عامي**: سكّن نهايات الكلمات سكونها العامي وأثبت وقف نهايات الأسطر (إلا القافية الممدودة فأثبت مدّها)، والتنوين لا يُغنّى عامياً فاستبدله بحركة أو سكون وقف.
   - في الحالين: **الشدّة تُكتب صريحة دائماً مع حركتها** (لا شدّة عارية)، وهمزة الوصل تُحرَّك كما تُنطق في سياقها المغنّى.
٤. **احترم تشكيل المستخدم المسبق**: كلمة وصلتك مشكّلة تشكيلاً كاملاً تبقى بتشكيلها حرفياً (فهي غالباً تصحيح نطق متعمد من صاحبها) — لا تغيّره إلا لخطأ نطق قاطع في اللهجة، واذكره في issues.
٥. **حافظ حرفياً** على: عدد المقاطع وأنواعها ومددها وترتيبها، عدد الأسطر وعدد كلمات كل سطر، الوزن والقافية، والمعنى. لا تضف ولا تحذف ولا تعِد صياغة.
٦. أدرج في issues أبرز ما صححته (إملاء أو تشكيل يغيّر المعنى أو مخالفة نطق لهجي) مع السبب — حتى ١٠ فقط.
