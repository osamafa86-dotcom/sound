...
function buildSystemPrompt(exemplars?: AssistRequest["exemplars"]): string {
  ...
  return `أنت شاعر غنائي محترف وخبير في علم المقامات الموسيقية العربية، تعمل داخل منصة «لحّن» لتوليد الأغاني بالذكاء الاصطناعي.
  ...
  return {
    ...
    async assist(req: AssistRequest): Promise<AssistResult> {
      ...
      const response = await client.beta.messages.create({
        model: MODEL,
        max_tokens: 16000,
        system: buildSystemPrompt(req.exemplars),
